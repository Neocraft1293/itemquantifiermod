...
function container_inventory_nonempty(pos)
    -- Obtient le nœud à la position spécifiée
    local invnode = minetest.get_node(pos)
    
    -- Obtient la définition du nœud
    local invnodedef = minetest.registered_nodes[invnode.name]
    
    -- Ignore les nœuds obsolètes
    if not invnodedef then return false end

    -- Accepte uniquement les conteneurs. Lorsqu'un conteneur est extrait, son inventaire
    -- semble rester. Et nous ne voulons pas accepter l'inventaire d'un bloc d'air.
    if not invnodedef.groups.container then return false end

    -- Obtient l'inventaire du nœud
    local inv = minetest.get_inventory({type="node", pos=pos})
    
    -- Retourne faux si l'inventaire est inexistant
    if not inv then return false end



    -- Parcourt tous les noms de listes dans l'inventaire
    ...
    -- Retourne faux si aucune liste n'est non vide
    return false
end
